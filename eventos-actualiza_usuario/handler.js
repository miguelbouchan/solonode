...
var connection = mysql.createConnection({
 ...
 if (event.nombre == (null || undefined) ||
    ...
    event.es_activo == (null || undefined) ||
  ...
  if (!expr.test(event.correo)) {
    ...
    connection.query("CALL ACTUALIZAR_USUARIO('"+event.nombre+"','"+event.apellido_paterno+"','"+event.apellido_materno+"',"+medad+",'"+event.correo+"',"+idrol+",'"+token+"','"+url_avatar+"','"+event.descripcion_rol+"','"+event.es_activo+"',"+event.telefono+","+event.id_estado+");", function (err2, rows) {
      if (err2) {
