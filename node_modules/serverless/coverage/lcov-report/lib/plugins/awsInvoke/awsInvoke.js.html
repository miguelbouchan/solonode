...
<html lang="en">
<head>
    <title>Code coverage report for lib/plugins/awsInvoke/awsInvoke.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../../index.html">all files</a> / <a href="index.html">lib/plugins/awsInvoke/</a> awsInvoke.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
  log(invocationReply) {
    const color = !invocationReply.FunctionError ? 'white' : <span class="branch-1 cbranch-no" title="branch not covered" >'red';</span>
&nbsp;
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (invocationReply.Payload) {
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (msg.startsWith('START') || msg.startsWith('END') || msg.startsWith('REPORT')) {
<span class="cstat-no" title="statement not covered" >        return chalk.gray(msg);</span>
      } else <span class="missing-if-branch" title="if path not taken" >I</span>if (msg.trim() === 'Process exited before completing request') {
<span class="cstat-no" title="statement not covered" >        return chalk.red(msg);</span>
      }
&nbsp;
      const splitted = msg.split('\t');
&nbsp;
      <span class="missing-if-branch" title="else path not taken" >E</span>if (splitted.length &lt; 3 || <span class="branch-1 cbranch-no" title="branch not covered" >new Date(splitted[0]) === 'Invalid Date')</span> {
        return msg;
      }
<span class="cstat-no" title="statement not covered" >      const reqId = splitted[1];</span>
<span class="cstat-no" title="statement not covered" >      const time = chalk.green(moment(splitted[0]).format(dateFormat));</span>
<span class="cstat-no" title="statement not covered" >      const text = msg.split(`${reqId}\t`)[1];</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      return `${time}\t${chalk.yellow(reqId)}\t${text}`;</span>
    };
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (invocationReply.LogResult) {
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Fri Jun 10 2016 16:00:38 GMT+0200 (EET)
</div>
</div>
